...
def init_instructions(win, basic_stim, ifi):
    ...
    En cada trial la asociación entre las teclas 'z' y 'm' y los símbolos que representan irá cambiando.  \
    FÍJATE BIEN antes de darle a la tecla de respuesta"
    nextt.text = "Pulsa spacio para continuar"
...
def main_instructions(win):
    ...
    querrá decir que lo estás haciendo bien"
    inst.draw()
    nextt.draw()
    win.flip()
    event.waitKeys(keyList = ["space"])
