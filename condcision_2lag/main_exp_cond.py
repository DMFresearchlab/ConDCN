...
# Experiment design
stimList = []
for ITI in [500, 3500]: # 75% vs 25% larger values than 1 in the CP & DP conditions correspond to one orientation
            stimList.append({'Length':ITI}) #, 'n_reps': n_reps
...
for thisBlock in range(main_exp['nblocks']): # iterate over blocks
    ...
    thr_trials_var = [['subj','nblock', 'ntrial', 'nrep', 'ITI_dur', 'cond', 'DV', 'resp', 'r_map', 'correct', 'RT']] # saving conditions here
    ...
    for thisTrial in trials: # will continue the staircase until it terminates!
        ...
        for i_rep in range(stim['nreps']):
            ...
            if i_rep == 0:
                stim['ISI1_frames'] = round((500+np.random.randint(100,200))/ifi)
            else:
                stim['ISI1_frames'] = round( (ITI_dur+np.random.randint(100,200))/ifi)  # fixation alone
            ...
            thr_trials_var.append([subj_id, thisBlock , trials.thisN, i_rep, ITI_dur, cond, x, thisResp[1], expInfo['resp_maps'][0], correct, rt_deci])# saving conditions here
