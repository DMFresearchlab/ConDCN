...
def stim_config(ifi):
    ...
    stim['nstimloc'] = 8 # number of gratings per sequence
...
def draw_basic(win, stim): # creates a dictionary with basic stimulus features
    ...
    basic_stim['circle'] = visual.Circle(win=win,lineWidth = 20, units="deg", radius=2, lineColor=[1, 1, 1],edges=64)
    basic_stim['linev'] = visual.Line(win=win,lineWidth = 20,units="deg",lineColor=[1, 1, 1],start = [0, -2],end = [0, 2])
    basic_stim['lineh'] = visual.Line(win=win,lineWidth = 20,units="deg",lineColor=[1, 1, 1],start = [-2, 0],end = [2, 0])
    basic_stim['feedback1'] = visual.Circle(win=win,units="deg", lineWidth = 20,radius=1, edges=64, color = [0.5, 0.5, 0.5], pos = [-4, -7])
    basic_stim['feedback2'] = visual.Circle(win=win,units="deg", lineWidth = 20,radius=1, edges=64, color = [0.5, 0.5, 0.5], pos = [0, -7])
    basic_stim['feedback3'] = visual.Circle(win=win,units="deg", lineWidth = 20,radius=1, edges=64, color = [0.5, 0.5, 0.5], pos = [4, -7])
    return basic_stim
...
def resp_mapping(win, resp_option_f, basic_stim,  resp_maps, rgbs): # pass resp_option drawinf functions maps the response options spatially 
    resp_option_f(win, basic_stim, resp_maps[0], rgbs[0], np.array([-8,0])) 
    resp_option_f(win, basic_stim, resp_maps[1], rgbs[1], np.array([8,0]))
    return
